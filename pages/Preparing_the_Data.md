...
The process of cleaning and preparing the data for EDA and model building are as follows. The datasets from each fiscal quarter in 2016 and first 3 quarters in 2017 are concatenated into one dataset called **acc_data**, which has only a subset of columns (to save memory). The reason only the first 3 fiscal quarters of 2017 are used instead of all 4 was so that we can get an equal amount of rows for the combined accepted loans and the individual rejected loans dataset. The combined accepted loan data set (a combination of 7 datasets) is over 700,000 rows, equal to that of the rejected dataset. The rejected loan dataset from the LendingClub website is read and stored into **rej_data**. 
